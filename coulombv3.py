...
def get_attraction(x1, x2,nip,ns):
    ...
    if (x1.x == x2.x and x1.y == x2.y):
        ...
        return ((x1.q * x2.q / (pow((sqrt(pow(x1.x - x2.x, 2) + pow(x1.y - x2.y, 2))),(2+dif)))))
...
def standard_deviation(charges):
    average = average_euclidian(charges)
    total_distance = 0
    total_points = 0
    for i in range(0, len(charges)):
        for j in range(0, len(charges)):
            dx = charges[i].x - charges[j].x
            dy = charges[j].y - charges[j].y
            dist = sqrt(dx * dx + dy * dy)
        if dist != 0:
...
#calculate the standard deviation of euclidian distances
deviation = standard_deviation(charges)
